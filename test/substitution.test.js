...
const substitutionModule = require("../src/substitution").substitution;
...
it("Should return false if given alphabet was not provided", () => {
  const expected = false;
  const actual = substitutionModule("message")
  expect(actual).to.equal(expected);
}); 
    ...
    it("Should return false if the given alphabet is not exactly 26 characters long", () => {
      const expected = false;
      const actual = substitutionModule("message", "amddxgm");
      expect(actual).to.equal(expected);
    });
